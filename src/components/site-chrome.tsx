    ...
    <header className="sticky top-0 z-40 backdrop-blur-md bg-background/80 border-b border-border">
      <div className="mx-auto max-w-7xl px-4 lg:px-8 flex items-center justify-between h-16">
        ...
        <nav className="hidden lg:flex xl:hidden items-center gap-3">
          ...
          <button
            onClick={() => setOpen((v) => !v)}
            className="text-[11px] font-mono border border-border rounded-full px-2.5 py-1 hover:bg-cream transition-colors"
          >
            More +
          </button>
        </nav>
        <button
          onClick={() => setOpen((v) => !v)}
          className="lg:hidden text-sm border border-border rounded-full px-3 py-1.5 hover:bg-cream transition-colors"
        >
          {open ? "Close" : "Menu"}
        </button>
      </div>
